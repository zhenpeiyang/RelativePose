...
class PeriodicCallback(Callback):
    ...
    def __init__(self, cb_loc, pstep, func=None):
        assert cb_loc == CallbackLoc.epoch_start or \
            cb_loc == CallbackLoc.epoch_end, 'Callback Location Error: '
    ...
    def run(self, context):
        callback_log('Trigger PeriodicCallback')
        if self._func:
            self._func(context)
        else:
